...
_EXPLICIT_NOTES: dict[str, str] = {
    "engine.type": "`tflm` is the vanilla TFLM baseline engine; `executorch` consumes an exported Cortex-M `.pte`.",
    "engine.backend": "TFLM: `reference` or `cmsis_nn`; ExecuTorch: `arm` or `ns`. helia-rt: free-form passthrough; `ethos_u` enables the Ethos-U NPU (Vela-compiled model required, NPU-equipped board only). helia-aot: `ethos_u` compiles the model's Vela ethos-u ops into NPU kernels (same requirements).",
