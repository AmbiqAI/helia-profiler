...
def _sources() -> list[Path]:
    """The CLI modules to parse, or a hard failure.

    A glob over a directory that has moved returns an empty list, and an audit
    of nothing agrees with everything: the cross-check would pass while
    auditing no command at all.
    """
    if not CLI_DIR.is_dir():
        raise FileNotFoundError(
            f"{CLI_DIR} is not a directory, so the source audit would compare "
            "cli.json against nothing and pass. Point CLI_DIR at the package's "
            "CLI modules."
        )
    sources = sorted(CLI_DIR.glob("*.py"))
    if not sources:
        raise FileNotFoundError(f"{CLI_DIR} holds no Python modules to audit.")
    return sources
