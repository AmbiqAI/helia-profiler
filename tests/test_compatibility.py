...
def test_baseline_has_no_unrelated_ref_drift() -> None:
    ...
    assert {project.name: project.ref for project in baseline.projects} == {
        ...
        # nsx-sensors v0.2.1 — first release where the INA228 driver is
        # correct end-to-end: SHUNT_CAL datasheet scaling (v0.2.0), plus
        # ADCRANGE moved to its real register (CONFIG bit 4 — v0.2.0 wrote
        # a VTCT bit, so range-1 calibrations came out 4x wrong), DEVICE_ID
        # rev-nibble masking, and conversion_ready reading CNVRF. Pinned
        # for power.driver: ina228 (issue #95).
        "nsx-sensors": "9c130859486327e9859f5fcc90a41171ef89ef7a",
    }
