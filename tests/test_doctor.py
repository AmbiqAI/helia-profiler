...
def test_collect_checks_reports_required_python_separately() -> None:
    with (
        ...
        patch(
            ...
            side_effect=lambda name: object() if name in {"neuralspotx", "pylink"} else None,
        ),
    ...
    assert required_python == [
        ("neuralspotx Python package", "neuralspotx", True),
        ("pylink Python package (RTT/SWO transport)", "pylink", True),
    ]
...
def test_collect_checks_marks_missing_required_python() -> None:
    ...
    assert required_python == [
        ("neuralspotx Python package", "neuralspotx", False),
        ("pylink Python package (RTT/SWO transport)", "pylink", False),
    ]
