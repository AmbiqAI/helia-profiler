---
title: "Toolchain Comparison"
description: "Goal: run the same model/engine/board through GCC, armclang, and ATfE to see how compiler choice affects cycle counts."
---
...
[Toolchains](../../guide/toolchains/)) and run `hpx doctor` to check the
...
See [Engines → heliaAOT](../../guide/engines/#heliaaot).
...
flip `--engine` (see [Engine Comparison](../engine-comparison/)). To isolate
placement effects, also hold `--arena-location` / `--weights-location`
constant (see [Memory Placement Tuning](../../guides/memory-placement-tuning/)).

## Where to go deeper

- [Toolchains](../../guide/toolchains/) — install steps and trade-offs.
- [Engine Comparison](../engine-comparison/) — engine-only sweep.
- [Per-Layer Breakdown](../per-layer/) — drill into individual ops.
