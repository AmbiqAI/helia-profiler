...
description: "Goal: capture every available PMU counter (CPU, memory, MVE) for each layer in a model, not just the default CPU counter selection."
---
...
counter — see [Multi-pass profiling](../../guide/pmu-counters/#multi-pass-profiling)
...
- [PMU Counters](../../guide/pmu-counters/) — counter groups, aggregation,
  derived metrics (L1D hit rate, MVE instruction share), and how to read
  `profile_results.csv` with pandas.
- [Model Explorer Overlays](../../guide/model-explorer/) — visualize
  per-layer hot spots on the model graph.
- [Basic Profiling](../basic-profiling/) — the minimal-counter starting point.
