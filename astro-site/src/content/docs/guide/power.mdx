...
description: "heliaPROFILER can capture current, voltage, and energy alongside PMU data, using a GPIO-gated clean window: the firmware runs a dedicated, uninstrumented…"
---
...
:::caution[Don't power the EVB from two sources]
...
:::caution[Registry defaults are not always the verified wiring]
...
:::caution[The Adafruit board's 15 mΩ shunt is sized for amps, not milliamps]
...
:::caution[The MikroE Power Monitor Click has no onboard shunt]
...
:::caution[That duration comes from the firmware, not the instrument]
...
:::caution[On-target monitoring is inside its own measurement]
...
:::caution[`joulescope: device not found`]
...
:::caution[GPIO gate rose but did not fall (`no_gate_fall`)]
...
:::caution[Wrong `io_voltage` or wrong input index]
...
:::caution[`ready_observed: false` or `gated_window_duration_ratio` far from 1.0]
...
:::caution["gated avg current &lt;= whole-capture avg" warning]
...
:::caution[`driver: ondevice` raises `PowerError: not yet implemented`]
...
:::caution[TOPS-per-Watt missing from summary]
...
:::caution[Power numbers differ between transports]
