---
title: "PMU Counters"
description: "heliaPROFILER captures per-layer hardware performance counters — cycles, instructions, cache activity, and (on Cortex-M55) MVE/Helium vector unit activity.…"
---
...
- **Model Explorer overlay** — one `me_overlay_<COUNTER>.json` per counter
  ...
  the model graph. See [Model Explorer Overlays](../model-explorer/).
...
:::caution[`overflow_detected: true` in `summary.json`]
...
:::caution[All PMU counter selections silently ignored]
...
:::caution[Per-layer cycles read back as 0 on Apollo4]
...
:::caution[Requesting `all` counters takes a long time]
...
:::caution[Unknown PMU counter name]
...
:::caution[PMU counter group not supported for this target]
