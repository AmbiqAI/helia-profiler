...
description: "heliaPROFILER currently exposes four inference engines. Each profiling run uses exactly one engine — you choose which at configuration time. There is no \"run…"
---
...
[ExecuTorch NS Tier-1 Kernels](../../guides/executorch-ns-kernels/) for
measured deltas.
...
[memory guide](../memory/#placement-models-heliaaot-vs-executorch) for the
...
:::caution[AmbiqAI ns-cmsis-nn fork required]
...
[engine-comparison example](../../examples/engine-comparison/) for a repeatable
walkthrough.
