...
description: "heliaPROFILER uses a two-level hardware model: Board → SoC. The board identifies the physical EVB and dictates J-Link device strings, USB IDs, and pin…"
---
...
:::caution[Experimental Atomiq110 support]
...
See [Architecture → Adding an Engine](../../architecture/adding-an-engine/)
for the analogous engine path.

See the [Platform API reference](../../reference/api/platform/) for the
