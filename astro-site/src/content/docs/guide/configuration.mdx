---
title: "Configuration"
description: "heliaPROFILER uses a layered configuration system: a YAML file merged with CLI flags, resolved once at startup into an immutable config object."
---
...
[Configuration Reference](../../reference/configuration/). Calling
heliaPROFILER from Python directly? See the
[Python API configuration reference](../../reference/api/config/) for
...
8. PMU counter groups and selections. See [PMU Counters](../pmu-counters/).
9. Per-layer breakdown (vs. whole-model aggregate).
10. Inference iterations per PMU pass (averaged in results).
11. Enable Joulescope power capture. See [Power Measurement](../power/).
12. Output format: `csv` or `json`.
13. Generate Model Explorer overlay JSONs. See [Model Explorer](../model-explorer/).
...
generated [Configuration Reference](../../reference/configuration/). The notes
