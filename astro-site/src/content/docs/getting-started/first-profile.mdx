---
title: "First Profile"
description: "A guided, no-config first run: from unboxing an Apollo EVB to a per-layer cycle-count table in about five commands."
---
...
- [Installed heliaPROFILER and the toolchain](../install/) — `hpx doctor`
...
All required rows should show `✓` (see [Installation](../install/) if not).
...
software from [Installation](../install/#4-segger-j-link-software) is
installed.
...
[Output & Reports](../../guide/output/) for the full column reference and
what each counter means. The same data is also rendered as
[Model Explorer overlays](../../guide/model-explorer/) under
...
- **Repeatable runs?** → [Quick Start with a config file](../quickstart/)
- **Different model placement?** → [Memory Placement](../../guide/memory/)
- **Try heliaAOT?** → [Inference Engines](../../guide/engines/)
- **Faster build with armclang?** → [Toolchains](../../guide/toolchains/)
- **Add power numbers?** → [Power Measurement](../../guide/power/)

## Troubleshooting the first run

:::caution[`hpx doctor` reports missing tools]
See [Installation](../install/). Required: `arm-none-eabi-gcc`, `cmake`,
...
on your setup, switch to USB CDC — see [Transports](../../guide/transports/).
:::

:::caution[`overflow_detected: true` in summary]
