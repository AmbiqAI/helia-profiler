---
title: "Quick Start"
description: "Move from one-off CLI invocations to a YAML config you can check into git and re-run later. This page assumes you've already completed your first profile."
---
...
[your first profile](../first-profile/).
...
[Configuration](../../guide/configuration/) for the full schema.
...
2.  Arena and weights placement. Pin explicitly to `tcm`, `sram`, `mram`, or
    ...
    See [Memory Placement](../../guide/memory/).
3.  heliaRT library variant. `release-with-logs` keeps SWO printf available
    for debugging; `release` is leaner.
4.  Toolchain. See [Toolchains](../../guide/toolchains/) for `armclang` and
    `atfe` setup.
5.  Capture transport. See [Transports](../../guide/transports/).
...
See [Toolchains](../../guide/toolchains/) for what each toolchain costs to
...
[Power Measurement](../../guide/power/).

## What's next?

- [Configuration](../../guide/configuration/) — full YAML schema reference
- [Inference Engines](../../guide/engines/) — RT vs AOT vs TFLM trade-offs
- [Toolchains](../../guide/toolchains/) — GCC vs armclang vs ATfE
- [Examples](../../examples/) — end-to-end recipes for common scenarios
