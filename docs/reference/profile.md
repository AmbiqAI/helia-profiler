...
| `--power-duration` | int | auto | Upper bound on the power capture in seconds. A gated capture raises it to fit the planned window; the profiling window settings set the window length. |
