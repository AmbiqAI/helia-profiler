...
| `HPX_ERROR=stimer_dead` | all | `use_stimer_window` (helia-aot, helia-rt, tflm); `busy_loop_probe` (executorch) | `transport_control` | `protocol` | The 32.768 kHz XTAL failed the settle-and-verify probe at hpx_stimer_init(): the STIMER window clock is dead or implausible. Emitted BEFORE the window opens so the failure is attributed to the crystal instead of completing a window into the frozen-clock check (#110). Value: `stimer_dead settle_us=<n>`. Host hint: yes. |
