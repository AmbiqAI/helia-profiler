...
| `HPX_NPU` | helia-aot, helia-rt, tflm | `has_ethos_u` | `unconsumed` | `diagnostic` | Ethos-U NPU initialised; emitted once after nsx_npu_init succeeds. Value: `ethos-u85 init=ok`. |
...
| `HPX_ERROR=npu_init_failed` | helia-aot, helia-rt, tflm | `has_ethos_u` | `transport_control` | `protocol` | Ethos-U NPU bring-up (nsx_npu_init) failed before any engine work. Value: `rc=<n> hint=bitstream_sdk_generation_mismatch`. Host hint: yes. |
