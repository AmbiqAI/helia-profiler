...
| `HPX_ERROR=operator_count_exceeds_capacity` | executorch | `not power_only`; runtime: the capacity was actually exceeded during the pass | `transport_control` | `protocol` | The model has more operators than the per-layer record array holds. Value: `capacity=<n>`. Host hint: yes. Printed just before the CSV body, so the rows that follow are truncated rather than absent. |
