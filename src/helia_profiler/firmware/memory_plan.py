"""Resolve firmware placement and account planned memory against SoC capacity."""

from __future__ import annotations

import logging

from ..config import DEFAULT_ARENA_SIZE_BYTES, ModelConfig, TargetConfig, Transport
from ..errors import PlatformError
from ..engines import EngineType
from ..engines.base import EngineAdapter, EngineArtifacts, ExecutorchArtifacts
...
# Logical arena/weights region → physical region
...
def select_memory_plan(
    *,
    ...
    model: ModelConfig,
    artifacts: EngineArtifacts | None,
    arena_region: Placement,
    weights_region: Placement,
) -> MemoryPlan:
    """Prefer the engine plan; synthesize runtime buffers except for failed AOT extraction."""
    ...
    if engine_type is EngineType.HELIA_AOT:
        ...
        return MemoryPlan(engine=engine_type)
    return _synthesise_plan(engine_type, model, artifacts, arena_region, weights_region)
...
def _synthesise_plan(
    ...
    """Account engine runtime buffers in their resolved firmware regions."""
    arena = int(model.arena_size or DEFAULT_ARENA_SIZE_BYTES)

    try:
        model_bytes = int(model.path.stat().st_size)
    except OSError:
        model_bytes = 0
    ...
    if engine_type is EngineType.EXECUTORCH and isinstance(artifacts, ExecutorchArtifacts):
        ...
        # add_hpx_owned_consumers accounts PMU records for every engine.
    else:
    ...
    regions = tuple(
        MemoryRegionUsage(
            region=name,
            capacity=0,  # filled by apply_capacities
    ...
    for region_name, field in _REGION_FIELDS.items():
        ...
        if existing is not None:
            if existing.capacity not in (0, cap_bytes):
                ...
                log.warning(
                    "%s capacity: engine says %d B, SoC layout says %d B — using the SoC layout.",
                    region_name,
                    existing.capacity,
                    cap_bytes,
                )
...
def validate_memory_plan(plan: MemoryPlan) -> None:
    ...
    lines = [
        f"  {r.region}: {r.used} B used > {r.capacity} B capacity (over by {r.used - r.capacity} B)"
        for r in offenders
    ]
    detail = "\n".join(lines)
    first = offenders[0]
...
def add_hpx_owned_consumers(
    plan: MemoryPlan, *, soc: SocDef | None, engine_type: EngineType, target: TargetConfig
) -> MemoryPlan:
...
def resolve_placement(
    *, model: ModelConfig, board: str, soc: SocDef | None, adapter: EngineAdapter
) -> tuple[Placement, Placement]:
    """Resolve engine defaults and explicit overrides, rejecting unavailable regions."""
    ...
    arena_size = int(model.arena_size or DEFAULT_ARENA_SIZE_BYTES)
    try:
        model_size = int(model.path.stat().st_size)
    except OSError:
        model_size = 0
