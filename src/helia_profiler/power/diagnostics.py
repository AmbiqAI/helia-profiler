...
def longest_accepted_window_s(
    *,
    ...
    """Longest gate the duration check, or cross-boot drift, still accepts."""
...
def gate_fall_wait_s(
    ...
    *,
    ...
    when it cannot hold ``longest_window_s`` plus headroom. Without lock-step
    the wait starts at reset rather than GO, so boot and ``pre_window_s`` of
    warm-up precede the window. An unknown window keeps ``configured_s``.
    """
    if longest_window_s is None or longest_window_s <= 0:
        return configured_s
    minimum_s = longest_window_s + FALL_WAIT_HEADROOM_S
    if not lockstep:
        minimum_s += BOOT_SETTLE_S + pre_window_s
...
FROZEN_WINDOW_CLOCK_HINT = (
    ...
    "32.768 kHz XTAL source is stopped or absent on this board, leaving the "
    "counter at zero; check that the target populates and starts XT. Since "
    "#110 a dead crystal normally announces itself BEFORE the window "
    "(power-terminal phase stimer_dead) — reaching this hint without that "
    "report points at a crystal that stopped after the settle probe, or at a "
    "lost line on a lossy transport. In internal (on-device monitor) mode the "
    "same clock times the denominator for average power and current, so both "
    "are wrong by the same factor and only integrated energy and charge "
    "survive. In external mode the instrument owns those numbers and only the "
    "firmware-reported durations are affected."
)
