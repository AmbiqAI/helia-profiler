...
_c("ARM_PMU_MVE_LDST_UNALIGNED_NONCONTIG_RETIRED",  0x0298, "mve", "MVE unaligned non-contiguous load/store retired")
...
def get_counter(name: str) -> PmuCounter:
    ...
    if ctr is None:
        raise ValueError(
            f"Unknown PMU counter '{name}'.  "
            f"Available: {', '.join(sorted(_COUNTERS))}"
        )
    return ctr
