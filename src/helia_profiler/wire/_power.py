...
_TERMINAL_FIELDS: tuple[tuple[PowerTerminalKey, bool | None, str, str], ...] = (
    (PowerTerminalKey.TERMINAL_VERSION, True,
     "Envelope version; anything but 1 is refused.", "int"),
    (PowerTerminalKey.STATUS, True,
     "Whether the power run completed.", "ok | error"),
    (PowerTerminalKey.REQUESTED_COUNT, True,
     "Inferences the host asked for.", "int"),
    (PowerTerminalKey.COMPLETED_COUNT, True,
     "Inferences actually completed inside the gate.", "int"),
    (PowerTerminalKey.ELAPSED_US, True,
     "Device-measured duration of the gated window.", "microseconds"),
    (PowerTerminalKey.FINAL_PHASE, True,
     "Last phase the firmware reached (names the failure on error).", "string"),
    (PowerTerminalKey.ERROR_CODE, True,
     "Non-zero iff status is error.", "int"),
    (PowerTerminalKey.GATE_ASSERTED, True,
     "The firmware raised the GPIO gate.", "0 | 1"),
    (PowerTerminalKey.GATE_LOWERED, True,
     "The firmware lowered the gate again — the capture is bounded.", "0 | 1"),
    (PowerTerminalKey.MEASUREMENT_SOURCE, False,
     "Which on-device monitor produced the measurement.", "ina228"),
    (PowerTerminalKey.MEASUREMENT_SCOPE, False,
     "What the measurement covers.", "fixed_n_inference"),
    (PowerTerminalKey.ENERGY_NJ, False,
     "Energy accumulated across the gated window.", "nanojoules"),
    (PowerTerminalKey.MEASUREMENT_DURATION_US, False,
     "Measurement duration; must equal ELAPSED_US.", "microseconds"),
    (PowerTerminalKey.MEASUREMENT_COUNT, False,
     "Inferences covered; must equal COMPLETED_COUNT.", "int"),
    (PowerTerminalKey.MEASUREMENT_OVERFLOW, False,
     "The accumulator overflowed and the energy is not trustworthy.", "0 | 1"),
    (PowerTerminalKey.CHARGE_NC, False,
