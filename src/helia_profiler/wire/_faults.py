...
ERROR_SPECS: tuple[WireSpec, ...] = (
    ...
    _spec(
        error_token(FirmwareErrorCode.NPU_INIT_FAILED),
        WireKind.ERROR,
        "Ethos-U NPU bring-up (nsx_npu_init) failed before any engine work.",
        WireConsumer.TRANSPORT_CONTROL,
        WireCriticality.PROTOCOL,
        engines=NPU_ENGINES,
        condition=GATE_HAS_ETHOS_U,
        value_shape="rc=<n> hint=bitstream_sdk_generation_mismatch",
        has_host_hint=True,
    ),
    _spec(
